                         ...
                         int DP10_open_time)
{
  ...
  if (type == 2) // Custom Button Configuration
  {
    ...
    if ((MP3_num > 181 && MP3_num < 226) || (MP3_num >= 1 && MP3_num < 6)) // Valid Custom Sound Range Selected - Play Custom Sound Selection
    {
...
void moveUtilArm(int arm, int position)
{
  switch (arm)
  {
    ...
    case UTIL_ARM_BOTTOM:
      if (position == utilArmClosedPos)
      {
        btnCircle_MD_func = 39;
        isUtilArmBottomOpen = false;
      } else
      {
        btnCircle_MD_func = 38;
